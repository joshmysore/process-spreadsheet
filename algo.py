...
def process_spreadsheet():
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            print(f'No se puede abrir "{file_path}". Saltando este archivo. ')
            continue
        ...
        try:
            ...
            print(f'No se puede leer "{file_path}". Error: {str(e)}. Saltando este archivo.')
            continue
        ...
        for sheet_name, df in dfs.items():
            if not set(required_columns).issubset(df.columns):
                print(f'Hoja "{sheet_name}" en "{file_path}" no contiene las columnas requeridas. Saltando este archivo.')
                dfs.pop(sheet_name)
        ...
        for sheet_name, df in dfs.items():
            ...
            if df['Habitaciones'].dtype not in ['int64', 'float64'] or df['Baños'].dtype not in ['int64', 'float64']:
                print(f'Hoja "{sheet_name}" en "{file_path}" contiene los datos incorrectos para  "Habitaciones" y/o "Baños". Saltando este archivo.')
                dfs.pop(sheet_name)
                continue
    ...
    for file_path in file_paths:
        ...
        print(f'Procesamiento de datos finalizado. Resultados guardados como "{processed_file_name}"')
